...
while abs(Qtheta - QthetaOld) > convergenceThreshold
    ...
    for k = 1:nPaths
        % For each sampled trajectory, compute the local trajectory cost
        [Stheta(k, :), ~] = stompTrajCost(robot_struct, theta_samples{k}, R, voxel_world);

    end

    %% TODO: Given the local traj cost, update local trajectory probability

    P = stompUpdateProb(Stheta);

    
    %% TODO: Compute delta theta (aka gradient estimator, the improvement of the delta)
    dtheta = stompDTheta(P, em);

    %Apply smoothing matrix M to dtheta
    dtheta_smoothed = zeros(numJoints, nDiscretize);
    dtheta_smoothed(:,2:end-1) = dtheta(:,2:end-1) * M';

    % Update theta with the smoothed gradient estimator
